...
class Gr00tPolicy(BasePolicy):
    ...
    def _get_action_with_rtc(
        ...
        prev_actions_dict = {}
        prev_actions_np = prev_actions.detach().cpu().numpy()
        prev_actions_dict['left_hand'] = prev_actions_np[:, :, 0:7]
        prev_actions_dict['right_hand'] = prev_actions_np[:, :, 7:14]
        prev_actions_dict['left_arm'] = prev_actions_np[:, :, 14:21]
        prev_actions_dict['right_arm'] = prev_actions_np[:, :, 21:28]
        prev_actions_dict['rpy'] = prev_actions_np[:, :, 28:31]
        prev_actions_dict['height'] = prev_actions_np[:, :, 31:32]
        prev_actions_dict['torso_vx'] = prev_actions_np[:, :, 32:33]
        prev_actions_dict['torso_vy'] = prev_actions_np[:, :, 33:34]
        prev_actions_dict['torso_vyaw'] = prev_actions_np[:, :, 34:35]
        prev_actions_dict['target_yaw'] = prev_actions_np[:, :, 35:36]
        normalized_prev_actions_dict = self.processor.state_action_processor.apply_action(prev_actions_dict, self.embodiment_tag.value)
        normalized_prev_actions = []
        for key in normalized_prev_actions_dict.keys():
            normalized_prev_actions.append(normalized_prev_actions_dict[key])

        normalized_prev_actions = np.concatenate(normalized_prev_actions, axis=-1)
        assert normalized_prev_actions.shape[0] ==1 and normalized_prev_actions.shape[2] == 36
        model_pred = self.model.get_action_with_rtc(**collated_inputs, prev_actions=torch.from_numpy(normalized_prev_actions).to("cuda:0"), inference_delay=inference_delay, execution_horizon=execution_horizon, mask_schedule=mask_schedule, guidance_weight=guidance_weight)
        normalized_action = model_pred["action_pred"].float()
        ...
        for k in self.modality_configs["state"].modality_keys:
            batched_states[k] = np.stack([s[k] for s in states], axis=0)  # (B, T, D)
        unnormalized_action = self.processor.decode_action(
            normalized_action.detach().cpu().numpy(), self.embodiment_tag, batched_states
        )
