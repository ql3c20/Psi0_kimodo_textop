...
def get_checkpoint_path(output_dir: str, checkpoint_prefix: str = "checkpoint") -> str | None:
    ...
    if list(pathlib_dir.glob("config.json")):
        ...
        try:
            ...
            # Assume we have only store one checkpoint
            # if len(checkpoints_sorted) > 1:
            # #   print(checkpoints_sorted[-1][1])
            # #   os.remove(checkpoints_sorted[-1][1])
            #   import shutil
            #   shutil.rmtree(checkpoints_sorted[-1][1])
            #   return checkpoints_sorted[-2][1], True
...
def prepare_config_for_training(
    ...
    if model_args.traj_decoder is not None:
        # Explicit override for traj decoder (allow rebuild from scratch).
        ...
        config.proprio_size = model_args.proprio_size
        config.use_proprio = model_args.use_proprio
        config.sep_proprio = model_args.sep_proprio
        config.sep_query_token = model_args.sep_query_token
        ...
        config.action_output_ee_rot_dim = model_args.traj_action_output_ee_rot_dim
        config.action_output_hand_dim = model_args.traj_action_output_hand_dim
        config.ee_rot_representation = model_args.ee_rot_representation
    elif getattr(config, "traj_decoder_cfg", None) is None:
        config.traj_decoder_cfg = model_args.traj_decoder
    ...
    config.next_token_loss_coeff = training_args.next_token_loss_coeff
    config.ee_loss_coeff = training_args.ee_loss_coeff
    config.ee_2d_loss_coeff = training_args.ee_2d_loss_coeff
    config.ee_rot_loss_coeff = training_args.ee_rot_loss_coeff
    config.loss_use_l1 = training_args.loss_use_l1
    config.hand_loss_coeff = training_args.hand_loss_coeff
    config.hand_loss_dim = training_args.hand_loss_dim
    config.hand_kp_loss_coeff = training_args.hand_kp_loss_coeff
    config.use_movement_mask = training_args.use_movement_mask
    config.ee_movement_loss_weight = training_args.ee_movement_loss_weight
    config.kl_weight = training_args.kl_weight
