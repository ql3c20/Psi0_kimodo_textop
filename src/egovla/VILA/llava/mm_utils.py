...
def dynamic_process_images_and_prompt(images, prompt, data_args, image_folder=None):
    prompt = prompt.split(DEFAULT_IMAGE_TOKEN)
    idx = 0
    all_images = []
    for img in images:
        processed_images = process_image(img, data_args, image_folder, enable_dynamic_res=True)
...
def augment_img(image):
  ...
  aug_transformation = Compose(aug_list_of_transformations)
  augmented_img = aug_transformation(image)
  return augmented_img


def process_image(image_file, data_args, image_folder, enable_dynamic_res=False):
    ...
    if data_args.image_aspect_ratio == "dynamic" and enable_dynamic_res:
        assert crop_size["height"] == crop_size["width"]
        images = dynamic_preprocess(
            image, min_num=data_args.min_tiles, max_num=data_args.max_tiles, image_size=crop_size["height"]
        )
...
def process_image_ndarray(image_array, data_args, return_info=False, enable_dynamic_res=False):
    processor = data_args.image_processor
    image = Image.fromarray(cv2.cvtColor(image_array,  cv2.COLOR_BGR2RGB))

    if data_args.with_aug:
...
def process_image_bytes(image_bytes, data_args, reverse_channel_order=True, enable_dynamic_res=False):
    processor = data_args.image_processor

    # didn't do it when generating the dataset -> Need to do it here.
...
def process_image_ndarray_v2(image_array, data_args, reverse_channel_order=True, enable_dynamic_res=False):
    processor = data_args.image_processor
...
def process_images_ndarray(images, image_processor, model_cfg, enable_dynamic_res=False):

    model_cfg.image_processor = image_processor
    new_images = [process_image_ndarray(
        image, model_cfg, None, enable_dynamic_res=enable_dynamic_res
    ) for image in images]
