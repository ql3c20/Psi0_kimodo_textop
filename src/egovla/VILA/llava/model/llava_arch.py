...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        if len(cfgs) == 4:
            llm_cfg, vision_tower_cfg, mm_projector_cfg, traj_decoder_cfg = cfgs
        elif len(cfgs) == 3:
            llm_cfg, vision_tower_cfg, mm_projector_cfg = cfgs
            traj_decoder_cfg = ""
        else:
            raise ValueError("`llm_cfg` `mm_projector_cfg` `vision_tower_cfg` `traj_decoder_cfg` not found in the config.")
        ...
        self.vision_tower = build_vision_tower(vision_tower_cfg, config)
        self.mm_projector = build_mm_projector(mm_projector_cfg, config)
        self.traj_decoder = build_traj_decoder(traj_decoder_cfg, config)
    ...
    def load_pretrained(cls, model_path_or_config, *args, **kwargs):
        ...
        if len(cfgs) == 4:
            ...
            raise ValueError("`llm_cfg` `mm_projector_cfg` `vision_tower_cfg` `traj decoder` not found in the config.")
        ...
        vlm.traj_decoder = build_traj_decoder(traj_decoder_cfg, config)
    ...
    def save_pretrained(self, output_dir, state_dict=None, **kwargs):
        ...
        if safe_serialization:
            try:
                ...
                save_safetensors(state_dict, osp.join(output_dir, "model.safetensors"))
            except Exception:
                torch.save(state_dict, osp.join(output_dir, "pytorch_model.bin"))
        else:
            torch.save(state_dict, osp.join(output_dir, "pytorch_model.bin"))
        ...
        if self.get_traj_decoder():
            print(f"saving traj decoder to {osp.join(output_dir, 'traj_decoder')}")
            self.traj_decoder.config._name_or_path = osp.join(output_dir, "traj_decoder")
            traj_decoder_state_dict = OrderedDict(
                {k.split("traj_decoder.")[-1]: v for k, v in state_dict.items() if "traj_decoder" in k}
            )
            self.traj_decoder.save_pretrained(
                os.path.join(output_dir, "traj_decoder"),
                state_dict=traj_decoder_state_dict,
            )
            self.config.traj_decoder_cfg = self.traj_decoder.config
    ...
    def get_traj_decoder(self):
        traj_decoder = getattr(self, "traj_decoder", None)
        if type(traj_decoder) is list:
            traj_decoder = traj_decoder[0]
        return traj_decoder
    ...
    def post_config(self):
        ...
        if getattr(self.config, "mm_projector_cfg", None) is None:
            self.config.mm_projector_cfg = self.mm_projector.config
        if getattr(self.config, "traj_decoder_cfg", None) is None:
            self.config.traj_decoder_cfg = self.traj_decoder.config
    ...
    def encode_images(self, images):
        ...
        # print("Image Projection:", image_features.shape)
